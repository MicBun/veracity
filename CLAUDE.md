...
Amanah is an AI triage copilot for donation-crowdfunding trust & safety. A two-stage Claude pipeline screens every campaign for fraud/policy/zakat-eligibility signals, cites its evidence, and calibrates confidence — then a human reviewer makes every decision. Deployed on Vercel; data in Neon Postgres.
...
**Prompts are versioned** (`PROMPT_VERSION` in `lib/ai/prompts.ts`). Any prompt change must bump this constant — it is recorded on every `ai_assessment` and `eval_run` so metric changes are attributable. Current version is `v3` (tuned v1→v3 against the eval set; v4 was tried and reverted). The 7-category risk rubric lives in both the prompts and `RISK_FLAGS`; keep them aligned.
